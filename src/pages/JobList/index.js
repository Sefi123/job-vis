...
import { fetchActiveJob, fetchTaggedJobs } from "../../services/jobs";
...
const JobList = () => {
  ...
  useEffect(() => {
    if (activeField?.length) fetchJobsByField(1, activeField);
  }, [activeField]);
          ...
          {tags.map((tag, index) => (
            <div
              ...
              className={`tag-container ${
                tag?.title == activeField ? "selected-tag" : ""
              }`}
              onClick={() => {
                setActiveField(tag?.title);
              }}
            >
              {tag?.icon}
              <div>{tag?.title}</div>
